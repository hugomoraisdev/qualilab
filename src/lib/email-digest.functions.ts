...
export const runEmailDigest = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    ...
    const [
      ...
      supabaseAdmin
        .from("calibrations")
        .select("id,equipment_id,next_due_date,responsible_id")
        .lte("next_due_date", addDays(30))
        .is("deleted_at", null),
      supabaseAdmin.from("equipments").select("id,name,code,responsible_id").is("deleted_at", null),
      supabaseAdmin
        .from("action_plans")
        .select("id,description,responsible_id,deadline,status,origin_type")
        .lte("deadline", addDays(3))
        .is("deleted_at", null),
      supabaseAdmin
        .from("competencies")
        .select("id,user_id,skill,area,expires_at")
        .lte("expires_at", addDays(60))
        .is("deleted_at", null),
      supabaseAdmin
        .from("risks")
        .select("id,code,description,responsible_id,status")
        .is("deleted_at", null),
    ...
    const alertCfg: AlertRecipientsConfig = {
      calibracao: [],
      competencia: [],
      risco: [],
      fornecedor: [],
      documento: [],
      acao: [],
      ...((alertRecipientsRow?.value ?? {}) as Partial<AlertRecipientsConfig>),
    };
    ...
    for (const p of profiles ?? []) {
      if (p.email) {
        profileByName.set(p.name, p.email);
    ...
    // Envia o mesmo alerta para todos os usuários configurados na categoria
    function addAlertToConfigRecipients(catIds: string[], alert: DigestAlert) {
      for (const uid of catIds) {
        const email = profileById.get(uid);
        const name = profileByIdName.get(uid) ?? "";
    ...
    for (const cal of calibrations ?? []) {
      ...
      // + destinatários globais configurados para calibração
      addAlertToConfigRecipients(alertCfg.calibracao, alert);
    }
    ...
    for (const ap of actionPlans ?? []) {
      ...
      if (email) addAlert(email, recipientName, alert);
      // + destinatários globais configurados para ações
      addAlertToConfigRecipients(alertCfg.acao, alert);
    }
    ...
    for (const comp of competencies ?? []) {
      ...
      if (email) addAlert(email, name, alert);
      // + destinatários globais configurados para competências
      addAlertToConfigRecipients(alertCfg.competencia, alert);
    }
    ...
    for (const risk of risks ?? []) {
      ...
      if (email) addAlert(email, recipientName, alert);
      // + destinatários globais configurados para riscos
      addAlertToConfigRecipients(alertCfg.risco, alert);
    }
    ...
    for (const row of docMetaRows ?? []) {
      ...
      for (const [stageKey, stageData] of Object.entries({ elaboration, review, approval }) as [
        string,
        WorkflowStage,
      ][]) {
        ...
        const recipientName =
          ...
          "";
        if (email) addAlert(email, recipientName, alert);
        // + destinatários globais configurados para documentos
        addAlertToConfigRecipients(alertCfg.documento, alert);
      }
    }

    // Documentos de fornecedor vencendo (≤30 dias)
    // Destinatários: lista configurada; fallback para todos admins/gestores
    const fornecedorRecipients: { email: string; name: string }[] =
      alertCfg.fornecedor.length > 0
        ? alertCfg.fornecedor
            .map((uid) => ({
              email: profileById.get(uid) ?? "",
              name: profileByIdName.get(uid) ?? "",
            }))
            .filter((r) => r.email)
        : adminEmails;
