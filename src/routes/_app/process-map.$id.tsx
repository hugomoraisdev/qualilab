...
function ProcessDetailPage() {
  ...
  const removeStep = (sid: string) =>
    update({
      steps: draft.steps.filter((s) => s.id !== sid).map((s, i) => ({ ...s, order: i + 1 })),
    });
      ...
      <Tabs defaultValue="overview">
        <TabsList className="mb-4 flex-wrap h-auto">
          <TabsTrigger value="overview">
            <Workflow className="size-3 mr-1" /> Geral
          </TabsTrigger>
          <TabsTrigger value="io">
            <ArrowDown className="size-3 mr-1" /> Entradas/Saídas
          </TabsTrigger>
          <TabsTrigger value="steps">
            <GripVertical className="size-3 mr-1" /> Etapas / Fluxo
          </TabsTrigger>
          <TabsTrigger value="links">
            <FileText className="size-3 mr-1" /> Vínculos
          </TabsTrigger>
          <TabsTrigger value="actions">
            <Activity className="size-3 mr-1" /> Ações
          </TabsTrigger>
          <TabsTrigger value="history">
            <History className="size-3 mr-1" /> Histórico
          </TabsTrigger>
        </TabsList>
        ...
        <TabsContent value="io">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <IOList
              title="Entradas"
              items={draft.inputs}
              onAdd={(v) => addItem("inputs", v)}
              onRemove={(i) => removeItem("inputs", i)}
            />
            <IOList
              title="Saídas"
              items={draft.outputs}
              onAdd={(v) => addItem("outputs", v)}
              onRemove={(i) => removeItem("outputs", i)}
            />
          </div>
        </TabsContent>
            ...
            <div className="flex items-center justify-between mb-3">
              ...
              <div className="flex gap-2">
                ...
                >
                  {flowView ? (
                    <>
                      <LayoutList className="size-3" /> Editar
                    </>
                  ) : (
                    <>
                      <Network className="size-3" /> Visualizar fluxo
                    </>
                  )}
                </Button>
                {!flowView && (
            ...
            {flowView ? (
              ...
              draft.steps.length === 0 ? (
                <div className="text-sm text-muted-foreground italic py-8 text-center">
                  Nenhuma etapa cadastrada. Clique em "Editar" para adicionar.
                </div>
              ) : (
                <div className="flex flex-col items-center gap-0 py-4 overflow-x-auto">
                  ...
                  {draft.inputs.length > 0 && (
                    <>
                      <div className="rounded-full border-2 border-primary bg-primary/10 px-5 py-2 text-xs font-semibold text-primary max-w-xs text-center">
                        ENTRADAS: {draft.inputs.join(" · ")}
                      </div>
                      <div className="flex flex-col items-center">
                        <div className="w-px h-6 bg-border" />
                        <div className="border-l-4 border-l-transparent border-r-4 border-r-transparent border-t-8 border-t-muted-foreground/40" />
                      </div>
                    </>
                  )}
                  ...
                  {draft.steps.map((s, idx) => (
                    <div key={s.id} className="flex flex-col items-center w-full max-w-md">
                      <div className="w-full border-2 border-border rounded-lg bg-background shadow-sm p-4 relative">
                  ...
                  {draft.outputs.length > 0 && (
                    <>
                      ...
                      <div className="rounded-full border-2 border-emerald-500 bg-emerald-50 dark:bg-emerald-950/30 px-5 py-2 text-xs font-semibold text-emerald-700 dark:text-emerald-400 max-w-xs text-center">
                        SAÍDAS: {draft.outputs.join(" · ")}
                      </div>
                    </>
                  )}
                </div>
        ...
        <TabsContent value="links">
          <div className="space-y-4">
            ...
            <LinkPicker
              ...
              options={risks.map((r) => ({
                id: r.id,
                label: `${r.code ?? r.id} — ${r.description}`,
              }))}
              selected={draft.linked_risk_ids}
              onToggle={(id) => toggleLink("linked_risk_ids", id)}
            />
            <LinkPicker
              title="Ocorrências / Não conformidades vinculadas"
              icon={AlertTriangle}
              options={occurrences
                .filter((o) => !o.deleted_at)
                .map((o) => ({
                  id: o.id,
                  label: `${o.code ?? o.id} — ${o.type} · ${o.description.slice(0, 60)}`,
                }))}
            ...
            <LinkPicker
              ...
              options={indicators
                ...
                .map((i) => ({ id: i.id, label: `${i.code ?? i.id} — ${i.name}` }))}
        ...
        <TabsContent value="actions">
          <LinkPicker
            title="Planos de ação relacionados"
            icon={Activity}
            options={actions.map((a) => ({
              id: a.id,
              label: `${a.code ?? a.id} — ${a.description.slice(0, 80)} (${a.status})`,
            }))}
            selected={draft.linked_action_ids}
            onToggle={(id) => toggleLink("linked_action_ids", id)}
          />
          <div className="text-xs text-muted-foreground mt-3">
            Use Planos de Ação para rastrear melhorias, correções e tratamentos relacionados a este
            processo.
            <Link to="/action-plans" className="text-primary hover:underline ml-1">
              Ir para planos →
            </Link>
          </div>
        </TabsContent>
          ...
          <div className="bg-card border border-border rounded-lg p-5">
            ...
            {draft.history.length === 0 ? (
              <div className="text-sm text-muted-foreground italic">
                Sem registros. Salve o processo para iniciar o histórico.
              </div>
            ) : (
              <ul className="space-y-2 text-xs">
                {draft.history.map((h) => (
                  <li key={h.id} className="border-l-2 border-primary/40 pl-3 py-1">
                    ...
                    {h.actor && (
                      <div className="text-muted-foreground">por {profileName(h.actor)}</div>
                    )}
                  </li>
                ))}
              </ul>
  ...
  return (
    <div className="bg-card border border-border rounded-lg p-5">
      ...
      <div className="max-h-64 overflow-y-auto border border-border rounded-md divide-y divide-border">
        {filtered.length === 0 && (
          <div className="p-3 text-xs text-muted-foreground italic">Nada encontrado.</div>
        )}
        {filtered.map((o) => {
