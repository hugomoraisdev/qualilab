...
function EqDetail() {
  ...
  async function saveLimit() {
    ...
    if (Number.isNaN(me) || !unit.trim()) {
      toast.error("Informe erro máximo e unidade.");
      return;
    }
  ...
  async function addCustomField() {
    ...
    const cf: EquipmentCustomField = {
      id: crypto.randomUUID(),
      label: cfLabel.trim(),
      value: cfValue,
    };
    await updateEquipmentMeta(id, (m) => ({ ...m, custom_fields: [...m.custom_fields, cf] }), {
      action: "Campo personalizado adicionado",
      detail: cf.label,
      actor,
    });
    setCfLabel("");
  ...
  return (
    <>
      ...
      <PageHeader
        title={e.name}
        description={`${e.code} · ${e.manufacturer ?? ""} ${e.model ?? ""}`.trim()}
        actions={<StatusBadge>{e.status}</StatusBadge>}
      />
      ...
      <Tabs defaultValue="dados" className="w-full">
        <TabsList>
          ...
          <TabsTrigger value="notificacoes">
            Notificações ({meta.notification_recipients.length})
          </TabsTrigger>
          <TabsTrigger value="campos">
            Campos personalizados ({meta.custom_fields.length})
          </TabsTrigger>
          ...
          <section className="bg-card border border-border rounded-lg p-5 shadow-sm space-y-4">
            <dl className="text-sm grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-1.5">
              <div className="flex justify-between">
                <dt className="text-muted-foreground">Categoria/Tipo</dt>
                <dd>{e.category ?? "—"}</dd>
              </div>
              <div className="flex justify-between">
                <dt className="text-muted-foreground">Série</dt>
                <dd className="font-mono text-xs">{e.serial_number ?? "—"}</dd>
              </div>
              <div className="flex justify-between">
                <dt className="text-muted-foreground">Setor/Localização</dt>
                <dd>{e.location ?? "—"}</dd>
              </div>
              <div className="flex justify-between">
                <dt className="text-muted-foreground">Aquisição</dt>
                <dd>{e.acquisition_date ?? "—"}</dd>
              </div>
              <div className="flex justify-between">
                <dt className="text-muted-foreground">Próxima calibração</dt>
                <dd>{e.next_calibration_date ?? "—"}</dd>
              </div>
              <div className="flex justify-between">
              ...
              <div className="flex justify-between items-center md:col-span-2 border-t border-border pt-2 mt-1">
                <dt className="text-muted-foreground shrink-0">Responsável</dt>
                <dd className="flex items-center gap-2">
                  <Select
                    value={e.responsible_id ?? "__none__"}
                    onValueChange={async (v) => {
                      await saveEquipment({ ...e, responsible_id: v === "__none__" ? null : v });
          ...
          <section className="bg-card border border-border rounded-lg p-5 shadow-sm space-y-4">
            <div>
              ...
              <p className="text-sm">
                {meta.calibration_limit ? (
                  <>
                    ± <span className="font-mono">{meta.calibration_limit.maxError}</span>{" "}
                    {meta.calibration_limit.unit}
                  </>
                ) : (
                  <span className="text-muted-foreground italic">
                    usando limite global por código (fallback)
                  </span>
                )}
              </p>
            </div>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
              <div className="space-y-1.5">
                <Label className="text-xs">Erro máximo</Label>
                <Input
                  type="number"
                  step="0.0001"
                  value={maxError}
              ...
              <div className="space-y-1.5">
                ...
                <Input
                  ...
                  placeholder="ex: g, mL, °C"
                />
              </div>
              <div className="flex items-end">
                <Button onClick={saveLimit} className="w-full">
                  Salvar limite
                </Button>
              </div>
            </div>
            <p className="text-xs text-muted-foreground">
              Esses valores serão usados automaticamente no formulário de nova calibração para
              aprovar/reprovar pontos.
            </p>
          </section>
        </TabsContent>
