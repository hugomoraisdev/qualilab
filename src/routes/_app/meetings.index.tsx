...
function MeetPage() {
  ...
  const rows = meetings
    ...
    .map((m) => {
      ...
      const allParticipants = [...m.participants, ...(meta?.participants ?? []).map((p) => p.name)];
      return {
        id: m.id,
        type: m.type,
        date: m.meeting_date,
        ...
        agenda: agenda
          ...
          .map((a) => a.title)
          .join("; "),
        participants_text: allParticipants.join(", "),
        status: derived,
    ...
    .filter(
      (r) =>
        !participant ||
        r.participants.some((p) => p.toLowerCase().includes(participant.toLowerCase())),
    );
  ...
  return (
    <>
      ...
      <DataTable
        data={rows}
        searchKeys={["id", "type", "agenda", "status", "sector", "participants_text"]}
        hideNew
        exportName="reunioes"
        ...
        columns={[
          ...
          },
          {
            ...
            render: (r) => <span className="text-xs">{r.participants.length} pessoas</span>,
          },
          {
            key: "agenda",
            header: "Pauta",
            render: (r) => (
              <span className="max-w-md truncate inline-block text-xs">{r.agenda}</span>
            ),
          },
          {
            key: "status",
            header: "Status",
            render: (r) => <StatusBadge tone={tone(r.status) as never}>{r.status}</StatusBadge>,
          },
        ]}
      />
    </>
