...
function FormBuilder() {
  ...
  const save = async () => {
    ...
    const approverNames = approverIds.map((id) => profiles.find((p) => p.id === id)?.name ?? id);
    const form: FormRow = {
      id: newId("FORM"),
      title: title.trim(),
      ...
      approvers: requiresApproval ? approverNames : [],
      status: "active",
    };
    await saveForm(form);
  ...
  return (
    <>
      ...
      <PageHeader
        ...
        description="Crie campos personalizados de forma autônoma — sem necessidade de TI."
      />
        ...
        <section className="bg-card border border-border rounded-lg p-5 shadow-sm space-y-4">
          <div className="grid sm:grid-cols-2 gap-3">
            <div className="space-y-1.5">
              ...
              <Input
                ...
                placeholder="Ex: Inspeção de Bancada"
              />
            </div>
            <div className="space-y-1.5">
              ...
              <Input
                ...
                placeholder="Curta explicação do uso"
              />
            </div>
          </div>
          ...
          <div className="border-t border-border pt-4 space-y-3">
            <div className="flex items-center gap-2 text-sm font-semibold">
              <Link2 className="size-4" /> Vínculos com módulos
            </div>
            <p className="text-xs text-muted-foreground">
              Selecione os módulos aos quais este formulário poderá ser vinculado em cada resposta.
            </p>
          ...
          <div className="border-t border-border pt-4 grid sm:grid-cols-2 gap-3">
            <div className="space-y-1.5">
              <Label className="text-xs flex items-center gap-1.5">
                <Clock className="size-3" /> Prazo (dias após envio)
              </Label>
              <Input
                type="number"
                min={0}
          ...
          {requiresApproval && (
            <div className="space-y-1.5">
              ...
              <div className="flex flex-wrap gap-1.5">
                {profiles.map((p) => {
                  const active = approverIds.includes(p.id);
                  return (
                    <button
                      key={p.id}
                      type="button"
                      onClick={() => toggleApprover(p.id)}
                      className={`text-xs px-2.5 py-1 rounded-full border ${active ? "bg-primary text-primary-foreground border-primary" : "border-border text-muted-foreground hover:border-primary/50"}`}
                    >
                      {p.name}
                    </button>
                ...
                {profiles.length === 0 && (
                  <span className="text-xs text-muted-foreground">Nenhum perfil disponível.</span>
                )}
              </div>
              {approverIds.length === 0 && (
                <p className="text-[11px] text-destructive">Selecione ao menos um aprovador.</p>
              )}
            </div>
          )}
              ...
              {fields.map((f, idx) => (
                <div key={f.id} className="border border-border rounded-md p-3 bg-background">
                  <div className="flex items-start gap-2">
                    ...
                    <button
                      onClick={() => remove(f.id)}
                      className="text-muted-foreground hover:text-destructive p-1.5 mt-1"
                    >
          ...
          <div className="border border-dashed border-border rounded-md p-4 space-y-3">
            ...
            {fields.length === 0 && (
              <div className="text-xs text-muted-foreground italic">
                Os campos aparecerão aqui conforme você adiciona.
              </div>
            )}
            {fields.map((f) => (
              <div key={f.id} className="space-y-1">
                ...
                {f.type === "textarea" && (
                  <textarea
                    className="w-full min-h-16 rounded-md border border-input bg-background px-2 py-1.5 text-xs"
                    disabled
                    placeholder="texto longo / observação"
                  />
                )}
                {f.type === "number" && (
                  <Input type="number" className="h-8" disabled placeholder="0" />
                )}
                ...
                {f.type === "radio" &&
                  (f.options ?? []).map((o, i) => (
                    <label key={i} className="flex items-center gap-1.5 text-xs">
                      <input type="radio" disabled name={`prev-${f.id}`} /> {o}
                    </label>
                  ))}
              </div>
            ))}
