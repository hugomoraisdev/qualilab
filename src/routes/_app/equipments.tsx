...
function EqPage() {
  ...
  const create = async () => {
    if (!draft.code.trim() || !draft.name.trim()) {
      toast.error("Informe o código e o nome do equipamento");
      return;
    }
  ...
  return (
    <>
      <PageHeader
        title="Equipamentos"
        description="Cadastro completo, calibração, manutenção e rastreabilidade"
      />
      <DataTable
        data={equipments}
        searchKeys={[
        ...
        columns={[
          ...
          { key: "location", header: "Setor/Localização", render: (r) => r.location ?? "—" },
          {
            key: "responsible_id",
            header: "Responsável",
            accessor: (r) => profiles.find((p) => p.id === r.responsible_id)?.name ?? "—",
            render: (r) =>
              profiles.find((p) => p.id === r.responsible_id)?.name ?? (
          ...
          },
          {
            ...
            header: "Próxima cal.",
            render: (r) => r.next_calibration_date ?? "—",
          },
          ...
          <div className="space-y-4 pt-2">
            ...
            <div className="space-y-1.5">
              ...
              <Select
                ...
                onValueChange={(v) => setDraft((p) => ({ ...p, responsible_id: v }))}
              >
                <SelectTrigger className="h-9 text-sm">
