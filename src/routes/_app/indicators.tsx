...
const emptyDraft = (): DraftIndicator => ({
  name: "", unit: "", target: "0", direction: "maior",
  area: "", frequency: "mensal", kind: "desempenho", process: "", responsible_id: "",
});
...
function IndicatorsPage() {
  ...
  const [resultDraft, setResultDraft] = useState<{ id: string; period: string; value: string; notes: string } | null>(null);
  ...
  const areas = useMemo(() => Array.from(new Set(list.map((i) => i.area).filter(Boolean))) as string[], [list]);
  const processes = useMemo(() => Array.from(new Set(Object.values(extras).map((e) => e.process).filter(Boolean))) as string[], [extras]);
  ...
  const overview = useMemo(() => {
    ...
    return { total: filtered.length, onTarget, off, noData,
  ...
  return (
    <>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 mb-5">
        ...
        <OverviewCard label="Abaixo da meta" value={overview.off} tone="warning" icon={TrendingDown} />
        <OverviewCard label="% no alvo" value={`${overview.pct}%`} tone="primary" icon={TrendingUp} />
      </div>

      {/* Filtros */}
      <div className="bg-card border border-border rounded-lg p-3 mb-5 flex flex-wrap items-end gap-2">
        <div className="flex items-center gap-1 text-xs text-muted-foreground"><Filter className="size-3" /> Filtros:</div>
        <select value={fKind} onChange={(e) => setFKind(e.target.value as "" | IndicatorKind)} className="h-8 rounded-md border border-input bg-background px-2 text-xs">
          <option value="">Todos os tipos</option>
          <option value="desempenho">Desempenho</option>
          <option value="qualidade">Qualidade</option>
        </select>
        <select value={fArea} onChange={(e) => setFArea(e.target.value)} className="h-8 rounded-md border border-input bg-background px-2 text-xs">
          <option value="">Todos os setores</option>
          {areas.map((a) => <option key={a} value={a}>{a}</option>)}
        </select>
        <select value={fProcess} onChange={(e) => setFProcess(e.target.value)} className="h-8 rounded-md border border-input bg-background px-2 text-xs">
          <option value="">Todos os processos</option>
          {processes.map((p) => <option key={p} value={p}>{p}</option>)}
        </select>
        <div className="flex items-center gap-1">
          <Label className="text-xs">De</Label>
          <Input type="month" value={fFrom} onChange={(e) => setFFrom(e.target.value)} className="h-8 w-32 text-xs" />
        </div>
        <div className="flex items-center gap-1">
          <Label className="text-xs">Até</Label>
          <Input type="month" value={fTo} onChange={(e) => setFTo(e.target.value)} className="h-8 w-32 text-xs" />
        </div>
        {(fKind || fArea || fProcess || fFrom || fTo) && (
          <Button variant="ghost" size="sm" onClick={() => { setFKind(""); setFArea(""); setFProcess(""); setFFrom(""); setFTo(""); }}>Limpar</Button>
        )}
      </div>
      ...
      {showForm && (
        <div className="bg-card border border-border rounded-lg p-5 shadow-sm mb-6 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 items-end">
          ...
          <div className="space-y-1.5">
            ...
            <select value={draft.kind} onChange={(e) => setDraft({ ...draft, kind: e.target.value as IndicatorKind })} className="h-9 w-full rounded-md border border-input bg-background px-3 text-sm">
              <option value="desempenho">Desempenho</option>
              <option value="qualidade">Qualidade</option>
            </select>
          </div>
          ...
          <div className="space-y-1.5"><Label className="text-xs">Setor / Área</Label><Input value={draft.area} onChange={(e) => setDraft({ ...draft, area: e.target.value })} placeholder="Ex: Produção" /></div>
          <div className="space-y-1.5"><Label className="text-xs">Processo</Label><Input value={draft.process} onChange={(e) => setDraft({ ...draft, process: e.target.value })} placeholder="Ex: Embalagem" /></div>
          <div className="space-y-1.5">
            <Label className="text-xs">Responsável</Label>
            <select value={draft.responsible_id} onChange={(e) => setDraft({ ...draft, responsible_id: e.target.value })} className="h-9 w-full rounded-md border border-input bg-background px-3 text-sm">
              <option value="">— Selecione —</option>
              {profiles.map((p) => <option key={p.id} value={p.id}>{p.name}</option>)}
            </select>
          </div>
          <div className="lg:col-span-4 flex justify-end gap-2"><Button variant="outline" onClick={() => setShowForm(false)}>Cancelar</Button><Button onClick={create}>Criar indicador</Button></div>
        </div>
      )}

      {filtered.length === 0 && (
        <div className="bg-card border border-dashed border-border rounded-lg p-8 text-center text-sm text-muted-foreground">
          {list.length === 0 ? 'Nenhum indicador cadastrado. Clique em "Novo indicador" acima.' : "Nenhum indicador corresponde aos filtros."}
        </div>
      )}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {filtered.map((i) => {
          const ex = getExtra(i.id);
          const allFor = allResults.filter((r) => r.indicator_id === i.id).sort((a, b) => a.period.localeCompare(b.period));
            ...
            <div key={i.id} className="bg-card border border-border rounded-lg p-5 shadow-sm">
              <div className="flex items-start justify-between mb-2 gap-2">
                ...
                <div className="flex flex-col items-end gap-1">
                  <span className={cn("rounded-full px-2 py-0.5 text-[10px] uppercase font-bold whitespace-nowrap",
                    ex.kind === "qualidade" ? "bg-primary/15 text-primary" : "bg-accent/40 text-accent-foreground")}>
                    {ex.kind}
                  </span>
                  <span className={cn("rounded-full px-2 py-0.5 text-[10px] uppercase font-bold whitespace-nowrap",
                    last ? (meetsTarget ? "bg-success/15 text-success" : "bg-warning/20 text-warning-foreground") : "bg-muted text-muted-foreground")}>
              ...
              <div className="flex items-end gap-1 mt-4 h-16">
                ...
                {results.length === 0 && <div className="text-xs text-muted-foreground italic">Sem resultados no período.</div>}
              </div>
              <div className="flex items-center justify-between mt-3 gap-2">
                <Button size="sm" variant="ghost" onClick={() => setHistoryId(isHistoryOpen ? null : i.id)}>
                  <History className="size-3" /> Histórico ({allFor.length})
                </Button>
                <Button size="sm" variant="outline" onClick={() => openResult(i)}><Plus className="size-3" /> Resultado</Button>
              </div>
              {isHistoryOpen && (
                      ...
                      <tbody>
                        {[...allFor].reverse().map((r) => (
                          <tr key={r.id} className="border-t border-border/40">
                            ...
                            <td className="text-right">{r.value} {i.unit}</td>
      ...
      {/* Modal de resultado */}
      {resultDraft && (
        <div className="fixed inset-0 z-50 grid place-items-center bg-black/40 p-4" onClick={() => setResultDraft(null)}>
          <div className="bg-card border border-border rounded-lg p-5 w-full max-w-md shadow-lg" onClick={(e) => e.stopPropagation()}>
            <div className="font-medium mb-3">Registrar resultado</div>
            <div className="space-y-3">
              <div className="space-y-1.5">
                <Label className="text-xs">Período</Label>
                <Input type="month" value={resultDraft.period} onChange={(e) => setResultDraft({ ...resultDraft, period: e.target.value })} />
              </div>
              <div className="space-y-1.5">
                <Label className="text-xs">Valor</Label>
                <Input type="number" step="any" value={resultDraft.value} onChange={(e) => setResultDraft({ ...resultDraft, value: e.target.value })} autoFocus />
              </div>
              <div className="space-y-1.5">
                <Label className="text-xs">Notas (opcional)</Label>
                <Input value={resultDraft.notes} onChange={(e) => setResultDraft({ ...resultDraft, notes: e.target.value })} />
              </div>
            </div>
            <div className="flex justify-end gap-2 mt-4">
              <Button variant="outline" onClick={() => setResultDraft(null)}>Cancelar</Button>
...
function OverviewCard({ label, value, tone, icon: Icon }: { label: string; value: number | string; tone: "default" | "success" | "warning" | "primary"; icon: React.ComponentType<{ className?: string }> }) {
  const toneCls = {
